...
class Client {
    ...
    constructor(options = {}) {
        ...
        OJS.net = {
            ...
            dataHandler(data) {
                ...
                switch(opcode) {
                    case OJS.options.opcode.setId: {
                        ...
                        OJS.util.log(`Joined world '${OJS.world.name}' and got id '${data.getUint32(1, true)}'`);
            ...
            messageHandler(data) {
                if(typeof data !== "string") return console.error("Client.net.messageHandler: data is not string.");
                if(data.startsWith("You are banned")) {
                    OJS.util.log("Got ban message.");
                    OJS.emit("destroy");
                    OJS.net.isWorldConnected = false;
                    OJS.net.isWebsocketConnected = false;
                ...
                if(data.startsWith("[D]")) OJS.util.log(`${nick}:${data.split(":").slice(1).join(":")}`);
                else if(data.startsWith("(M)")) OJS.util.log(`${nick}:${data.split(":").slice(1).join(":")}`);
                else if(data.startsWith("(A)")) OJS.util.log(`${nick}:${data.split(":").slice(1).join(":")}`);
                else if(data.startsWith("[") || /[0-9]/g.test(data[0])) OJS.util.log(`${nick}:${data.split(":").slice(1).join(":")}`);
                else if(data.startsWith("Server")) OJS.util.log(`${nick}:${data.split(":").slice(1).join(":")}`);
        ...
        void function makeSocket() {
            ...
            ws.onopen = () => {
                OJS.util.log("WebSocket connected!");
        ...
        if(options.unsafe) OJS.util.log("Using 'unsafe' option.");
        this._events = {};
		this._events_once = {};
    };
    on(event, fn) {
